...
public class Screen implements IRegion {
	...
	//private org.sikuli.script.Screen screen = null;
	...
	Screen(XmlRpcClient agent, int id) throws XmlRpcException, IOException {
		...
		this.curID = id;
		//*String scr = (String) agent.execute("Sikuli.getScreen", params);
		sXScreen = (String) agent.execute("Sikuli.getScreen", params);
		//*this.screen = (org.sikuli.script.Screen) Marshal.deserialize(scr);
	...
	public <PSRML> int click(PSRML target) throws Exception {
		...
		Vector<Object> params = new Vector<>();
		//params.addElement(Marshal.serialize(screen));
		params.addElement(sXScreen);
			
		if (target instanceof Pattern) {
			//*params.addElement(Marshal.serialize(((Pattern)target).getPattern()));
