...
public class Pattern {
	//private String sXPattern = null;
	private String imgpath = null;
	private BufferedImage bImage = null;
	float similarity = 0.95f;
	private Location offset = new Location(0, 0);
	//private org.sikuli.script.Pattern pattern = null;

	Pattern () {
		//Agent.log.info(">>");
		//*this.pattern = new org.sikuli.script.Pattern();
		//Agent.log.info("<<"); //Don't use this.pattern.toString() because of exception will be raised due to similar() method
	}

	/*public org.sikuli.script.Pattern getPattern() {
		Agent.log.info(">>");
		Agent.log.info("<< " + this.pattern.toString());
		return this.pattern;
	}*/
	...
	public Pattern (String imgURL) {
		...
		//*this.pattern = new org.sikuli.script.Pattern(imgURL);
		Agent.log.info("<< " + this.bImage.toString());
	}

	/*	Pattern (org.sikuli.script.Pattern ptn) {
		Agent.log.info(">> " + "ptn = " + ptn);
		this.pattern = ptn;
		Agent.log.info("<< " + this.pattern.toString());
	}*/
	...
	public Pattern similar(float similarity) {
		...
		//*org.sikuli.script.Pattern ptn = new org.sikuli.script.Pattern(this.pattern);	
		//*ret.pattern = ptn.similar(similarity);	
		//Agent.log.info("<< " + ret.pattern.toString());
	...
	public Pattern targetOffset(int dx, int dy) {
		...
		/*Pattern ret = new Pattern();
		org.sikuli.script.Pattern ptn = new org.sikuli.script.Pattern(this.pattern);	
		ret.pattern = ptn.targetOffset(dx, dy);*/
		offset.x = dx;
		offset.y = dy;
